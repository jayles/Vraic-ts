...
export default class MyContentTests {
	...
	public async globalSetup() {
		...
		// set input A
		await page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wci-a]') as HTMLInputElement).value = '');
		await page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wci-a]') as HTMLInputElement).focus());
		await page.keyboard.type(this.expectedA.toString());

		// set input B
		await page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wci-b]') as HTMLInputElement).value = '');
		await page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wci-b]') as HTMLInputElement).focus());
		await page.keyboard.type(this.expectedB.toString());

		// set input C
		await page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wci-c]') as HTMLInputElement).value = '');
		await page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wci-c]') as HTMLInputElement).focus());
	...
	public async checkPropCUpdates() {
		...
		const actualC: string = await this.page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wco-c]') as HTMLElement).innerHTML);
	...
	public async checkPropDhasValueHello() {
		// check outcome
		const actualD: string = await this.page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wco-d]') as HTMLElement).innerHTML);
	...
	public async checkAdditionTotalIs18() {
		// check outcome
		const additionTotal: string = await this.page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wco-total]') as HTMLElement).innerHTML);
	...
	public async checkMultiplicationTotalIs77() {
		// check outcome
		const additionTotal: string = await this.page.$eval(this.shadHostPath, el => (el.shadowRoot?.querySelector('[data-wco-mult]') as HTMLElement).innerHTML);
